...
function enhanceHandoffControls() {
  ...
  const existing = document.querySelector("#handoff-new-version");
  if (existing) {
    styleVersionButton(existing);
    return;
  }
...
import("./log-page-css.js?v=command-center-25").catch((error) => console.warn("Store Pilot log style failed", error));
import("./log-page-fixes.js?v=command-center-25").catch((error) => console.warn("Store Pilot log page fixes failed", error));
import("./log-final-polish.js?v=command-center-25").catch((error) => console.warn("Store Pilot final log polish failed", error));
