...
const TRUCK_ITEMS = [
  'Confirm coverage before truck work starts',
  'Clear safe path from front doors / delivery path',
  'Stage totes or carts before unloading',
  'Keep register/customer coverage protected',
  'Prioritize cooler, fresh, and customer-facing outs',
  'Check back room stacking and walking path',
  'Break down trash/cardboard as time allows',
  'Document anything delayed because of truck or coverage',
  'Tell Loretta/Richard what was not finished if needed'
];
...
function currentShiftFixed() {
  return normalizeShift(fixRead(SHIFT_FIX_KEY, '')) || activeShiftFromUi() || timeShift();
}
...
function openTruckDay() {
  ...
  panel.innerHTML = `<section class="truck-card" role="dialog" aria-modal="true"><div class="truck-head"><p>${currentShiftFixed()} shift</p><h2>Truck Day</h2></div><div class="truck-list">${TRUCK_ITEMS.map((item, index) => `<label class="truck-row"><input type="checkbox" data-truck-index="${index}" ${saved.checked?.[index] ? 'checked' : ''}><span>${truckEscape(item)}</span></label>`).join('')}<label class="truck-notes">Truck notes<textarea placeholder="Delivery time, what got finished, what had to wait, coverage issue...">${truckEscape(saved.notes || '')}</textarea></label></div><div class="truck-actions"><button type="button" data-truck-save>Save truck note</button><button type="button" data-truck-close>Close</button></div></section>`;
...
function addTruckTaskToShift() {
  ...
  const list = all[key] || [];
  if (!list.some((task) => /truck day/i.test(task.title || ''))) {
    list.unshift({ id: `truck-day-${day}-${shift}`, title: 'Truck Day receiving / staging', area: 'Truck', minutes: 45, detail: 'Protect coverage, stage product, and document what had to wait.' });
    all[key] = list;
...
function injectTruckTool() {
  ...
  tool.innerHTML = `<p>Optional shift tool</p><h3>Truck Day</h3><div><button type="button" data-truck-open>Open checklist</button><button type="button" data-truck-add>Add to shift</button></div>`;
  stage.prepend(tool);
}
